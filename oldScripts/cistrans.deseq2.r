...
# for some reason, not exactly the same, but proceed with PCA plots
...
dim(B10[B10$padj<0.05 & !is.na(B10$padj),]) # 4094    6
dim(B20[B20$padj<0.05 & !is.na(B20$padj),]) # 3033    6
...
dim(A10[A10$padj<0.05 & !is.na(A10$padj),]) # 5570
dim(A20[A20$padj<0.05 & !is.na(A20$padj),]) # 3578
...
# using pre-existing size factors
sizeFactors(dds2) <- coldataAll$lib_size/mean(coldataAll$lib_size)
# pairwise deseq workflow, c(maxxa, tx2094), ratio calculated as tx2094/maxxa
batchMT<- rbind(
...
dim(B.mt10[B.mt10$padj<0.05 & !is.na(B.mt10$padj),]) # 1991    6
dim(B.mt20[B.mt20$padj<0.05 & !is.na(B.mt20$padj),]) # 1223    6
...
dim(B.tm10[B.tm10$padj<0.05 & !is.na(B.tm10$padj),]) # 1940    6
dim(B.tm20[B.tm20$padj<0.05 & !is.na(B.tm20$padj),]) # 1439    6
...
sumT<-as.data.frame(matrix(unlist(lapply(list(res10,res.mt10, res.tm10,res20, res.mt20,res.tm20), function(x)table(x$category)) ),ncol=6))
rownames(sumT)<-names(table(res10$category))
names(sumT)<-c("F1 10dpa","MxT 10dpa", "TxM 10dpa","F1 20dpa","MxT 20dpa", "TxM 20dpa")
print(sumT)
...
###### Let us test effects of fold change cutoff
